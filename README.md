...
- **찾아낸 원인**: 5번 공정인 Mold mask layer 형성 단계의 Develop(현상) 과정에서 시간/농도가 부족했던 것이 근본 원인이었다. 현상이 충분히 이루어지지 않아 노출된 부분의 PR이 다 씻겨나가지 못하고 잔류했고, 이 잔여 PR이 이후 Cu 표면을 가려 etchant가 닿지 못하면서 Seed layer etching 단계에서 Cu가 그대로 남게 되었다.
- **배운 점**: 공정 중간 단계(Mold mask layer의 Develop)의 작은 시간/농도 부족이 몇 단계 뒤(Seed layer etching)에서야 문제로 드러날 수 있다는 것을 확인했다. 이후 공정에서 이상이 발견되면 바로 앞 단계만 볼 게 아니라, 공정을 역순으로 하나씩 되짚어보는 접근이 원인 파악에 효과적이었다.
